...
const ZEN_MODEL = process.env.OPENCODE_ZEN_MODEL || "deepseek-v4-flash";
...
// calling.
//
// Zen is now backed by an OpenCode Go subscription ($10/mo, real paid
// credits), not the shared/exhausted free pool from before — so it's
// restored as the primary brain. DeepSeek V4 Flash specifically: 1M context,
// confirmed native tool calling, and a generous Go-covered allowance
// (~31,650 req/5h), making it the strongest choice available right now.
// Groq (GPT-OSS) is the first fallback — your own quota, predictable.
const MODELS = {
  vision: process.env.VISION_MODEL || (GEMINI_API_KEY ? "gemini-3.6-flash" : "pixtral-large-latest"),
  agent: process.env.AGENT_MODEL || (HAS_ZEN ? `zen:${ZEN_MODEL}` : (HAS_GROQ ? "groq:openai/gpt-oss-120b" : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL))),
  conversation: process.env.CONVERSATION_MODEL || (HAS_GROQ ? "groq:openai/gpt-oss-20b" : (HAS_ZEN ? `zen:${ZEN_MODEL}` : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL))),
  tools: process.env.TOOLS_MODEL || (HAS_ZEN ? `zen:${ZEN_MODEL}` : (HAS_GROQ ? "groq:openai/gpt-oss-120b" : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL))),
...
//   1. OpenCode Zen DeepSeek V4 Flash (Go subscription credits — real paid
//      quota now, 1M context, strong tool calling, generous allowance)

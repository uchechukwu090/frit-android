...
// ======================= MODELS =======================
// Model resolution order: explicit per-role env override -> provider-specific
// default (Groq if GROQ_API_KEY, else Gemini if GEMINI_API_KEY, else Mistral).
// The legacy MISTRAL_MODEL / MISTRAL_FAST_MODEL vars are honored as the Mistral
// fallback so an existing .env (e.g. mistral-medium-latest) is actually used.
const MODELS = {
  vision: process.env.VISION_MODEL || (GEMINI_API_KEY ? "gemini-2.0-flash" : "pixtral-large-latest"),
  agent: process.env.AGENT_MODEL || (HAS_GROQ ? "groq:llama-3.3-70b-versatile" : (GEMINI_API_KEY ? "gemini-2.0-flash" : MISTRAL_MODEL)),
  conversation: process.env.CONVERSATION_MODEL || (HAS_GROQ ? "groq:llama-3.1-8b-instant" : (GEMINI_API_KEY ? "gemini-2.0-flash" : MISTRAL_MODEL)),
  tools: process.env.TOOLS_MODEL || (HAS_GROQ ? "groq:llama-3.3-70b-versatile" : (GEMINI_API_KEY ? "gemini-2.0-flash" : MISTRAL_MODEL)),
  coding: process.env.CODING_MODEL || (HAS_GROQ ? "groq:qwen/qwen3.6-27b" : "codestral-latest"),
  fast: process.env.FAST_MODEL || (HAS_GROQ ? "groq:llama-3.1-8b-instant" : (GEMINI_API_KEY ? "gemini-2.0-flash" : MISTRAL_FAST_MODEL)),
...
async function mistralTranscribe(audioBase64) {
  ...
  try {
    const res = await fetch("https://api.mistral.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: [{ type: "audio_url", audio_url: { url: `data:audio/wav;base64,${base64Data}` } }] },
        ],
        max_tokens: 800,
      }),
    });
    ...
    console.warn(`[MistralTranscribe] Mistral Voxtral failed: ${res.status}`);
...
app.post("/transcribe", async (req, res) => {
  try {
    ...
    const text = await mistralTranscribe(audio_base64);

...
async function fetchHighImpactEvents() {
  ...
  try {
    ...
    return high;
  } catch (err) { console.warn("[NewsFilter]", err.message); return []; }
}
...
async function checkNewsFilter(symbol) {
  ...
  if (nearby.length > 0) {
    return { blocked: true, reason: `High-impact news within 30 min: ${nearby.map(e => `${e.currency} ${e.title}`).join(", ")}`, events: nearby };
  }
...
async function analyzeSymbol(symbol, interval = "1h", customSize = null) {
  ...
  if (newsCheck.blocked) {
    return {
      symbol: sym, direction: "NEUTRAL", strength: "NEWS_BLACKOUT", interval: iv,
  ...
  const aiOpinion   = direction === "NEUTRAL"
    ? `No clear edge right now. Wait for cleaner structure.${auctionNote}`
    : `${direction} bias with ${strength.toLowerCase()} confidence. Structure=${structure.trend}, vol=${volatility.regime}, patterns=${patterns.join(", ") || "none"}.${auctionNote}`;

  const result = {
    symbol: sym, price, direction, strength, interval: iv, candleCount: candles.length,
    ...
    auction: auction ? {
      poc: +auction.poc.toFixed(dp), vah: +auction.vah.toFixed(dp), val: +auction.val.toFixed(dp),
      position: auctionSig.position, bias: auctionSig.bias, note: auctionSig.note,
...
async function sendToMT5Bridge({ symbol, action, lotSize, entry, sl, tp, reason = "" }) {
  ...
  } catch (err) { console.error("[MT5 Bridge]", err.message); return { mode: "live", status: "failed", error: err.message }; }
}
...
// Auto-start scanner in production
if (process.env.AUTO_SCANNER === "true") {
  frit.startScanner(2 * 60 * 1000); // 2-minute scan interval
...
async function runLocalTool(name, args = {}, agentState = null) {
  switch (name) {
    case "search_web":     return { ok: true,  await webSearch(args.query || "") };
    case "get_weather":    return { ok: true,  await getWeather(args.city || "Lagos") };
    case "get_market_data":return { ok: true,  await fetchMarketPrices([args.symbol || "BTCUSD"]) };
    case "analyze_market": return { ok: true,  await analyzeSymbol(args.symbol, args.interval, args.outputsize) };
    ...
    case "assert_text_visible": {
      if (!agentState || !agentState.deviceState) {
        return { ok: true,  { asserted: false, text_searched: args.text, error: "No device state available. Call read_screen first." } };
      }
      const text = String(args.text || "").toLowerCase();
      const screenText = String(agentState.deviceState?.screen_text || "").toLowerCase();
      const found = screenText.includes(text);
      return { ok: true,  { asserted: found, text_searched: args.text, screen_snippet: screenText.slice(0, 200) } };
    } 
...
function waitForAndroid(callId, timeoutMs = 60000) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      pendingAndroidCallbacks.delete(callId);
      reject(new Error(`Android timeout (${timeoutMs / 1000}s) for callId ${callId}`));
    }, timeoutMs);
...
app.get("/", (_req, res) => {
  res.json({
    name: "FRIT LLM Orchestrator & Market Risk Engine",
    description: "AI brain for the FRIT Android app. Handles LLM chat, market analysis, trade execution (via MT5 bridge), and Android automation orchestration. The Android app is required for device-control actions.",
    status: "online",
    endpoints: {
      ...
      utility: ["/weather"],
      enhanced: ["/enhanced/analyze", "/scanner/*", "/crash-gsri/*", "/acp/status", "/paper-trades/*", "/systems/status", "/pipeline/history"]
    },
  });
});
app.get("/health", (_req, res) => {
  res.json({ status: "active", models: MODELS, twelve_ !!TWELVE_DATA_KEY, mt5_bridge: !!MT5_BRIDGE_URL, sandbox_url: SANDBOX_URL, pending_android_callbacks: pendingAndroidCallbacks.size, frame_buffer: frameBuffer.length, cache_entries: _cache.size, uptime: Math.floor(process.uptime()) + "s" });
});
...
app.post("/automate", requireAuth, async (req, res) => {
  ...
  try {
    const result = await handleAutomationRequest({ goal, device_state, memory, history, max_steps, mode });
    res.json(result);
  } catch (err) { console.error("[/automate]", err.message); res.status(500).json({ error: "Automation failed", details: err.message }); }
});
...
app.get("/market/quote", async (req, res) => {
  try { const symbol = String(req.query.symbol || "BTCUSD").toUpperCase(); const data = await fetchMarketPrices([symbol]); res.json(data[symbol] || { error: "Not found" }); } catch (err) { res.status(500).json({ error: "Quote fetch failed", details: err.message }); }
});
app.post("/market/batch", async (req, res) => {
  try { const { symbols = [] } = req.body || {}; if (!Array.isArray(symbols) || !symbols.length) return res.status(400).json({ error: "symbols array required" }); res.json(await fetchMarketPrices(symbols.map(s => String(s).toUpperCase()))); } catch (err) { res.status(500).json({ error: "Batch fetch failed", details: err.message }); }
});
app.all("/market/analyze", requireAuth, async (req, res) => {
  try {
    const body = req.body || {}; const query = req.query || {};
    ...
    const outputsize = (req.method === "GET" ? query.outputsize : body.outputsize ?? query.outputsize) ? Number(req.method === "GET" ? query.outputsize : body.outputsize ?? query.outputsize) : null;
    res.json(await analyzeSymbol(symbol, interval, outputsize));
  } catch (err) { console.error("[/market/analyze]", err.message); res.status(500).json({ error: "Analysis failed", details: err.message }); }
});

// STEP 4 (OPTIONAL): ENHANCED TRADE ENDPOINT (Replaces original)
app.post("/trade", requireAuth, async (req, res) => {
  ...
  if (pipeline === "enhanced") {
    ...
    } catch (err) { console.error("[/trade enhanced]", err.message); return res.status(500).json({ error: "Enhanced trade failed", details: err.message }); }
  }
  ...
  } catch (err) { console.error("[/trade]", err.message); res.status(500).json({ error: "Trade failed", details: err.message }); }
});

// ==================== GSRI STATUS ====================
app.get("/gsri/status", requireAuth, async (req, res) => {
  try { const snap = await getGsriSnapshot(); const score = parseFloat(snap?.Risk_Score ?? 0.8); res.json({ snapshot: snap, lot_scale: gsriLotScale(score), entries_allowed: parseInt(snap?.Alert ?? 1) === 0 }); } catch (err) { res.status(500).json({ error: "GSRI status failed", details: err.message }); }
});
// ==================== TRADE MEMORY ====================
...
app.get("/weather", async (req, res) => {
  try { const city = String(req.query.city || "Lagos"); const result = await getWeather(city); res.json({ city, result }); } catch (err) { res.status(500).json({ error: "Weather failed", details: err.message }); }
});
// ==================== TRANSCRIBE ====================
app.post("/transcribe", async (req, res) => {
  try {
    ...
    form.append("file", buffer, { filename: "audio.webm", contentType: mime_type });
    form.append("model", MODELS.whisper);
    const out = await fetch("https://api.groq.com/openai/v1/audio/transcriptions", { method: "POST", headers: { Authorization: `Bearer ${GROQ_API_KEY}`, ...form.getHeaders() }, body: form });
    const data = await out.json();
    if (!out.ok) return res.status(500).json({ error: "Transcription failed", details: data });
    res.json({ text: data.text || "", model_used: MODELS.whisper });
  } catch (err) { console.error("[/transcribe]", err.message); res.status(500).json({ error: "Transcription failed", details: err.message }); }
});

// STEP 3: ENHANCED PIPELINE ROUTES
...
app.post("/paper-trades/resolve", requireAuth, (req, res) => {
  ...
  if (result) { res.json({ ok: true, trade: result }); } else { res.status(404).json({ error: "Trade not found" }); }
});
...
app.listen(PORT, () => {
  console.log(`╔══════════════════════════════════════════════════════════════════╗
║ 🤖 FRIT LLM Orchestrator & Market Risk Engine               ║
╠══════════════════════════════════════════════════════════════════╣
║ Port : ${String(PORT).padEnd(55)}║
║                                                                 ║
║ What this server actually does:                                 ║
║ ✅ LLM chat + agentic tool orchestration (Groq)                 ║
║ ✅ Market analysis engine (EMA/RSI/MACD/BB/ATR/VWAP/SR)         ║
║ ✅ Volume profile / market auction (POC, VAH, VAL, HVN, LVN)    ║
║ ✅ Multi-timeframe confirmation (4H bias)                       ║
║ ✅ News filter (FF calendar — 30 min blackout)                  ║
║ ✅ GSRI systemic risk overlay (blocks trades on alert)          ║
║ ✅ Lot size engine (per-pair pip math, JPY/crypto aware)        ║
║ ✅ MT5 bridge (live) or paper mode if bridge not configured     ║
║ ✅ Whisper STT transcription                                    ║
║ ✅ Screen frame ingestion → Llama-4-Scout vision analysis       ║
║ ✅ Android automation orchestration (State Machine: Verify-Act) ║
║ ✅ Code sandbox (Python / JavaScript via Sandbox service)       ║
║ ✅ ENHANCED: Autonomous Scanner, Crash GSRI, ACP & Pipeline     ║
║                                                                 ║
║ What requires the Android app:                                  ║
║ 📱 All device-control actions (tap, scroll, type, open, etc.)   ║
...
async function fetchMarketPrices(symbols = []) {
  ...
  await Promise.all(symbols.map(async sym => {
    ...
    const spot = await fetchSpotPrice(s);
    if (!spot) { result[s] = { symbol: s, price: 0, change24h: 0, currency: "USD", error: "Not found" }; return; }
    const change24h = spot.change24h !== undefined && spot.change24h !== 0
      ? spot.change24h
...
function analyzeStructure(candles, price) {
  ...
  return {
    trend,
    bos_bullish: lastHigh != null && prevHigh != null && lastHigh > prevHigh,
    bos_bearish: lastLow  != null && prevLow  != null && lastLow  < prevLow,
...
// ==================== GSRI RISK OVERLAY ====================
// GSRI snapshot is a JSON file written by the Python pipeline (run_gsri_and_commit.py).
// It is committed to the repo by GitHub Actions. The server reads it from disk or fetches
// it from the raw GitHub URL. If neither is available, a conservative fallback is used.

const GSRI_LOCAL_PATH = process.env.GSRI_SNAPSHOT_PATH || join(__dirname, "gsri_snapshot.json");
const GSRI_REMOTE_URL = process.env.GSRI_REMOTE_URL    || "";
const GSRI_ALPHA      = 0.6;          // sensitivity: higher = more lot reduction per unit of Risk_Score
const GSRI_MIN_SCALE  = 0.2;          // floor: never go below 20% of base lot
const GSRI_REMOTE_TTL = 5 * 60 * 1000;

let _gsriRemoteCache = null; // { snap, ts }

async function getGsriSnapshot() {
  // 1. Local file (fast, written by GitHub Actions CI)
  try {
    const raw  = readFileSync(GSRI_LOCAL_PATH, "utf8");
    const data = JSON.parse(raw);
    const snap = Array.isArray(data) ? data.at(-1) : data;
  ...
  // 2. Remote raw GitHub URL with 5-min cache
  if (GSRI_REMOTE_URL) {
    const now = Date.now();
    if (_gsriRemoteCache && now - _gsriRemoteCache.ts < GSRI_REMOTE_TTL) return _gsriRemoteCache.snap;
    try {
  ...
  // 3. Conservative fallback — treat as high-risk
  return { Risk_Score: 0.8, Alert: 1, source: "fallback" };
}
...
// ==================== ANDROID AGENT TOOLS ====================
// These are tool schemas sent to Groq for function-calling.
// Server-side tools (search_web, get_weather, get_market_data, analyze_market, run_code)
// execute here. All other tools produce a JSON command that the Android app must execute.
// The Android app calls /automation-results with the callId to complete the loop.

const SERVER_SIDE_TOOLS = new Set(["search_web", "get_weather", "get_market_data", "analyze_market", "run_code"]);

const AGENT_TOOLS = [
  // --- Navigation (Android executes) ---
  { type: "function", function: { name: "open_app",            description: "Launch any installed app by name.",                        parameters: { type: "object", properties: { app_name:    { type: "string" } }, required: ["app_name"] } } },
  { type: "function", function: { name: "open_url",            description: "Open a full URL in the default browser.",                  parameters: { type: "object", properties: { url:         { type: "string" } }, required: ["url"] } } },
  { type: "function", function: { name: "press_back",          description: "Press the Android back button.",                           parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "press_home",          description: "Press the Android home button.",                           parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "open_recents",        description: "Open the recent apps screen.",                             parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "open_notifications",  description: "Open the notification shade.",                             parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "get_current_app",     description: "Return the foreground package name and label.",            parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "get_current_activity",description: "Return the current Android activity name.",                parameters: { type: "object", properties: {} } } },
  // --- Screen reading (Android executes) ---
  { type: "function", function: { name: "read_screen",          description: "Read visible text and content descriptions from the screen.", parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "read_screen_structured",description: "Return a structured accessibility tree dump.",           parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "find_element",         description: "Find a UI element by text, hint, id, or class.",          parameters: { type: "object", properties: { query: { type: "string" } }, required: ["query"] } } },
  { type: "function", function: { name: "take_screenshot",      description: "Take a screenshot and return metadata.",                  parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "analyze_screenshot",   description: "Analyze the last screenshot visually with the vision model.", parameters: { type: "object", properties: { prompt: { type: "string" } }, required: ["prompt"] } } },
  // --- Touch (Android executes) ---
  { type: "function", function: { name: "tap_button",     description: "Tap an element by its visible label.",                         parameters: { type: "object", properties: { label: { type: "string" } }, required: ["label"] } } },
  { type: "function", function: { name: "tap_coordinates",description: "Tap the screen at exact x/y coordinates.",                    parameters: { type: "object", properties: { x: { type: "number" }, y: { type: "number" } }, required: ["x","y"] } } },
  { type: "function", function: { name: "double_tap",     description: "Double-tap by label or coordinates.",                         parameters: { type: "object", properties: { label: { type: "string" }, x: { type: "number" }, y: { type: "number" } } } } },
  { type: "function", function: { name: "long_press",     description: "Long-press by label or coordinates.",                         parameters: { type: "object", properties: { label: { type: "string" }, x: { type: "number" }, y: { type: "number" }, duration_ms: { type: "number" } } } } },
  { type: "function", function: { name: "scroll",         description: "Scroll the screen in a direction.",                           parameters: { type: "object", properties: { direction: { type: "string", enum: ["up","down","left","right"] } } } } },
  { type: "function", function: { name: "swipe",          description: "Swipe using start and end coordinates.",                      parameters: { type: "object", properties: { startX: { type: "number" }, startY: { type: "number" }, endX: { type: "number" }, endY: { type: "number" }, duration_ms: { type: "number" } }, required: ["startX","startY","endX","endY"] } } },
  { type: "function", function: { name: "drag_and_drop",  description: "Drag from one coordinate to another.",                        parameters: { type: "object", properties: { startX: { type: "number" }, startY: { type: "number" }, endX: { type: "number" }, endY: { type: "number" }, duration_ms: { type: "number" } }, required: ["startX","startY","endX","endY"] } } },
  // --- Text input (Android executes) ---
  { type: "function", function: { name: "focus_field", description: "Focus a text field by label, hint, or content-desc.", parameters: { type: "object", properties: { field: { type: "string" } }, required: ["field"] } } },
  { type: "function", function: { name: "type_text",   description: "Type text into the focused or targeted field.",      parameters: { type: "object", properties: { value: { type: "string" }, field: { type: "string" } }, required: ["value"] } } },
  { type: "function", function: { name: "clear_text",  description: "Clear the focused or targeted input field.",         parameters: { type: "object", properties: { field: { type: "string" } } } } },
  { type: "function", function: { name: "paste_text",  description: "Paste text via clipboard into a field.",             parameters: { type: "object", properties: { value: { type: "string" }, field: { type: "string" } }, required: ["value"] } } },
  { type: "function", function: { name: "press_enter", description: "Press Enter / Done / Search on the keyboard.",       parameters: { type: "object", properties: {} } } },
  { type: "function", function: { name: "hide_keyboard",description: "Hide the soft keyboard.",                           parameters: { type: "object", properties: {} } } },
  // --- System (Android executes) ---
  { type: "function", function: { name: "toggle_wifi",             description: "Toggle Wi-Fi on or off.",           parameters: { type: "object", properties: { enabled: { type: "boolean" } }, required: ["enabled"] } } },
  { type: "function", function: { name: "toggle_bluetooth",        description: "Toggle Bluetooth on or off.",       parameters: { type: "object", properties: { enabled: { type: "boolean" } }, required: ["enabled"] } } },
  { type: "function", function: { name: "set_volume",              description: "Set media volume 0–100.",           parameters: { type: "object", properties: { level: { type: "number" } }, required: ["level"] } } },
  { type: "function", function: { name: "set_brightness",          description: "Set screen brightness 0–100.",      parameters: { type: "object", properties: { level: { type: "number" } }, required: ["level"] } } },
  { type: "function", function: { name: "open_app_settings",       description: "Open settings page for an app.",   parameters: { type: "object", properties: { app_name: { type: "string" } }, required: ["app_name"] } } },
  { type: "function", function: { name: "grant_permission_if_prompted", description: "Handle Android permission prompts.", parameters: { type: "object", properties: { allow: { type: "boolean" } }, required: ["allow"] } } },
  // --- Communication (Android executes) ---
  { type: "function", function: { name: "make_call",    description: "Initiate a phone call.",                         parameters: { type: "object", properties: { contact_name: { type: "string" }, phone_number: { type: "string" } } } } },
  { type: "function", function: { name: "send_whatsapp",description: "Open WhatsApp for a contact and optional message.", parameters: { type: "object", properties: { contact_name: { type: "string" }, message: { type: "string" } }, required: ["contact_name"] } } },
  { type: "function", function: { name: "send_sms",     description: "Open SMS composer for a contact.",               parameters: { type: "object", properties: { contact_name: { type: "string" }, message: { type: "string" } }, required: ["contact_name"] } } },
  // --- Media / Productivity (Android executes) ---
  { type: "function", function: { name: "play_music",  description: "Play music on Spotify or YouTube.",          parameters: { type: "object", properties: { query: { type: "string" } }, required: ["query"] } } },
  { type: "function", function: { name: "navigate_to", description: "Open Maps and navigate to a destination.",   parameters: { type: "object", properties: { destination: { type: "string" } }, required: ["destination"] } } },
  { type: "function", function: { name: "take_photo",  description: "Open camera and capture a photo.",           parameters: { type: "object", properties: { front_camera: { type: "boolean" } } } } },
  { type: "function", function: { name: "set_alarm",   description: "Set an alarm.",                              parameters: { type: "object", properties: { label: { type: "string" }, time: { type: "string" } }, required: ["time"] } } },
  { type: "function", function: { name: "set_timer",   description: "Start a countdown timer.",                   parameters: { type: "object", properties: { duration: { type: "string" } }, required: ["duration"] } } },
  // --- Server-side tools (execute on this server) ---
  { type: "function", function: { name: "search_web",    description: "Search for current information on the web.",                parameters: { type: "object", properties: { query:    { type: "string" } }, required: ["query"] } } },
  { type: "function", function: { name: "get_weather",   description: "Get current weather for a city.",                          parameters: { type: "object", properties: { city:     { type: "string" } }, required: ["city"] } } },
  { type: "function", function: { name: "get_market_data",description: "Get live spot price for a forex/crypto symbol.",          parameters: { type: "object", properties: { symbol:   { type: "string" } }, required: ["symbol"] } } },
  { type: "function", function: { name: "analyze_market", description: "Full market analysis: structure, indicators, auction, trade plan.", parameters: { type: "object", properties: { symbol: { type: "string" }, interval: { type: "string" }, outputsize: { type: "number" } }, required: ["symbol"] } } },
...
// ==================== AUTOMATION HANDLER ====================
// Architecture note: this server is the AI brain. The Android app is the hands.
// For Android tools, the server generates a command and waits (up to 60s) for the
// app to call /automation-results to confirm execution. If the app is offline or
// crashes, the pending action times out and the agent records the failure.
...
// pending Android callbacks — keyed by callId
const pendingAndroidCallbacks = new Map();
...
async function handleAutomationRequest({ goal, device_state = {}, memory = [], history = [], max_steps = 8, mode = "auto" }) {
  ...
  const toolNames        = getToolsForTask(goal);
  const tools            = AGENT_TOOLS.filter(t => toolNames.includes(t.function.name) || SERVER_SIDE_TOOLS.has(t.function.name));
  const deviceStateText  = buildDeviceStateBlock(device_state);
  const memoryText       = buildMemoryBlock(summarizeMemory(memory, 6, 700));
  const systemPrompt     = [
    "You are FRIT, an Android device-control AI agent.",
    "Your role: translate the user's goal into precise, verifiable actions.",
    "After navigation or risky actions, always verify with read_screen or read_screen_structured.",
    "If you cannot find an element, try find_element, scrolling, or screenshot analysis.",
    "Never fabricate tool results.",
    "Server-side tools (search_web, get_weather, get_market_data, analyze_market, run_code) execute here.",
    "All other tools require the Android app to execute them and report back.",
    "", "Device state:", deviceStateText,
    memoryText ? `\n${memoryText}` : "",
  ].filter(Boolean).join("\n");
  ...
  for (let step = 0; step < Math.max(1, Math.min(max_steps, 10)); step++) {
    ...
    for (const tc of toolCalls) {
      ...
      if (SERVER_SIDE_TOOLS.has(name)) {
        ...
        // Android tool — wait for the app to execute and call back
        try {
          const deviceResult = await waitForAndroid(tc.id);
          androidActions.push({ id: tc.id, tool: name, arguments: args, device_result: deviceResult });
          messages.push({ role: "tool", tool_call_id: tc.id, content: JSON.stringify(deviceResult) });
...
// ==================== SCREEN FRAME INGESTION ====================
// The Android app sends base64 JPEG frames to /screen/frame.
// These are stored in a 10-frame rolling buffer for vision analysis.
// This is NOT screen casting. The server does not stream frames anywhere.
// It stores them so /screen/analyze-frame can pass them to Llama-4-Scout.
...
app.post("/chat", requireAuth, async (req, res) => {
  ...
  // Fast path: concise market signal (no LLM needed)
  if (symMatch && isMarketMessage(message) && !wantsDetailedTradeReason(message)) {
    try {
      const sym      = symMatch[1].toUpperCase();
      const iv       = ivMatch?.[1]?.toLowerCase() || "1h";
  ...
  try {
    ...
    res.json({
      ...
      model_used: model,
      payload_guard: { screen_context_used: !!safeScreenCtx, memory_items_used: safeMemory.length, history_messages_used: safeHistory.length },
    });
  } catch (err) {
...
// Called by the Android app after executing a tool command
app.post("/automation-results", requireAuth, (req, res) => {
  const { callId, status, observation } = req.body || {};
  if (!callId) return res.status(400).json({ error: "callId required" });
  const resolved = resolveAndroid(callId, { status: status || "ok", observation: observation || "" });
...
app.post("/plan", requireAuth, async (req, res) => {
  ...
  const prompt = [
    ...
    "", "Goal:", truncateText(task, 2000),
    "", "Device state:", buildDeviceStateBlock(device_state),
...
// ==================== SCREEN FRAME INGESTION ====================
// Android app sends frames here; they are stored for vision model analysis.
app.post("/screen/frame", requireAuth, (req, res) => {
  const { frameData, timestamp, width, height } = req.body || {};
  if (!frameData) return res.status(400).json({ error: "frameData required" });
  frameBuffer.push({ data: frameData, timestamp: timestamp || Date.now(), width, height });
...
app.post("/screen/analyze-frame", requireAuth, async (req, res) => {
  ...
  try {
    const out = await groqChat({
      model: MODELS.vision,
      messages: [{ role: "user", content: [{ type: "text", text: prompt || "Describe the UI elements and any actionable items on this screen." }, { type: "image_url", image_url: { url: `data:image/jpeg;base64,${frame.data}` } }] }],
      max_tokens: 500,
    });
    res.json({ analysis: out.choices[0].message.content, frameTimestamp: frame.timestamp, frameIndex: frameIndex >= 0 ? frameIndex : frameBuffer.length - 1 });
  } catch (err) {
...
app.get("/screen/status", requireAuth, (_req, res) => {
  res.json({ buffered_frames: frameBuffer.length, max_buffer: FRAME_BUFFER_SIZE, oldest_frame_ts: frameBuffer[0]?.timestamp || null, newest_frame_ts: frameBuffer.at(-1)?.timestamp || null });
});
...
app.post("/trade", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // 2. Technical analysis + news filter
    const analysis = await analyzeSymbol(symbol, interval);
...
app.get("/gsri/status", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ snapshot: snap, lot_scale: gsriLotScale(score), entries_allowed: parseInt(snap?.Alert ?? 1) === 0 });
  } catch (err) {
...
║  ✅ Whisper STT transcription                                     ║
║  ✅ Screen frame ingestion → Llama-4-Scout vision analysis        ║
║  ✅ Android automation orchestration (app is the hands)           ║
║  ✅ Code sandbox (Python / JavaScript via Sandbox service)        ║
║                                                                   ║
║  What requires the Android app:                                   ║
║  📱 All device-control actions (tap, scroll, type, open, etc.)    ║

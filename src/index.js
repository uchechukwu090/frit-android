...
const MODELS = {
  ...
  agent: process.env.AGENT_MODEL || (HAS_GROQ ? "groq:llama-3.3-70b-versatile" : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL)),
  conversation: process.env.CONVERSATION_MODEL || (HAS_GROQ ? "groq:llama-3.1-8b-instant" : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL)),
  tools: process.env.TOOLS_MODEL || (HAS_GROQ ? "groq:llama-3.3-70b-versatile" : (GEMINI_API_KEY ? "gemini-3.6-flash" : MISTRAL_MODEL)),
...
function buildFallbackChain(primary) {
  const chain = [primary];
  if (GEMINI_API_KEY && primary !== "gemini-3.6-flash") chain.push("gemini-3.6-flash");

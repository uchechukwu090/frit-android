...
const MODELS = {
  ...
  voxtral:      process.env.MISTRAL_VOXTRAL_MODEL       || "voxtral-mini-transcribe-realtime",
};
    ...
    const res = await fetch("https://api.mistral.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: [{ type: "audio_url", audio_url: { url: `data:audio/webm;base64,${base64Data}` } }] },
        ],
        max_tokens: 800,
      }),
    });
    if (!res.ok) throw new Error(`Mistral STT failed: ${res.status}`);
    const data = await res.json();
...
async function analyzeSymbol(symbol, interval = "1h", customSize = null) {
  ...
  const result = {
    ...
    auction: auction ? {
      ...
      hvn: auction.hvn.map(p => +p.toFixed(dp)), lvn: auction.lvn.map(p => +p.toFixed(dp)),
    } : null,
...
async function handleAutomationRequest({ goal, device_state = {}, memory = [], history = [], max_steps = 8, mode = "auto" }) {
  ...
  for (let step = 0; step < Math.max(1, Math.min(max_steps, 10)); step++) {
    const out      = await mistralChat({ model: pickModel({ mode, taskType }), messages, tools, tool_choice: "auto", max_tokens: 1500 });
...
// ==================== ROUTES ====================
app.get("/", (_req, res) => {
  res.json({ name: "FRIT SMC+CRT Trading Engine & Mistral AI Orchestrator", description: "Pure SMC+CRT + Volume Profile + GSRI — no indicators. Single Mistral API ecosystem.", status: "online", endpoints: { core: ["/health", "/chat", "/automate", "/automation-results", "/plan"], market: ["/market/quote", "/market/batch", "/market/analyze", "/trade", "/gsri/status"], scanner: ["/scanner/scan", "/scanner/control", "/scanner/status", "/smc/status"], memory: ["/memory/trade"], screen: ["/screen/frame", "/screen/analyze-frame", "/screen/status"], transcribe: ["/transcribe"], utility: ["/weather"] } });
});
...
app.post("/chat", requireAuth, async (req, res) => {
  ...
  try {
    const out = await mistralChat({ model, messages: [{ role: "system", content: systemPrompt }, ...safeHistory, { role: "user", content: userContent }], max_tokens: 1600 });
...
app.post("/plan", requireAuth, async (req, res) => {
  ...
  try {
    const out = await mistralChat({ model: MODELS.fast, messages: [{ role: "system", content: "You create precise Android automation plans." }, { role: "user", content: prompt }], max_tokens: 700 });
...
app.post("/screen/analyze-frame", requireAuth, async (req, res) => {
  ...
  try {
    const out = await mistralChat({ model: MODELS.vision, messages: [{ role: "user", content: [{ type: "text", text: prompt || "Describe the UI elements and any actionable items on this screen." }, { type: "image_url", image_url: { url: `data:image/jpeg;base64,${frame.data}` } }] }], max_tokens: 500 });
...
app.post("/trade", requireAuth, async (req, res) => {
  ...
  if (pipeline === "enhanced") {
    try {
      ...
      return res.json({ status: "submitted", pipeline: "smc_crt", symbol: symbol.toUpperCase(), action: result.decision === "BUY" ? "buy" : "sell", lotSize: result.lot_size, entry: result.entry, sl: result.sl, tp: result.tp, confidence: result.confidence, smc_crt: result.smc_crt, crash_phase: result.crash_phase, gsri_mode: result.gsri_mode, mt5_result: tradeResult });
    } catch (err) {
